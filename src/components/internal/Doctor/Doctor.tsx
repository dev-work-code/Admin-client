...
const DoctorsTable: React.FC = () => {
    ...
    if (error) return <p className="text-center text-red-500">Error loading doctors: {String(error)}</p>;
    ...
    const filteredDoctors = (filter === "all" ? allDoctors : data?.[filter] || []).filter(
        (doctor) =>
            (doctor.doctorName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                doctor.doctorMobileNumber?.includes(searchTerm) ||
                doctor.areaOfSpecialization?.toLowerCase().includes(searchTerm.toLowerCase())) ?? false
    );
    ...
    return (
        <Card className="p-6 space-y-4 max-w-5xl mx-auto shadow-[2px_4px_5px_0px_#E9EBFFB2] rounded-[38px] border">
            <CardTitle className="text-2xl font-medium mb-6 ml-6 text-[#003CBF]">Doctors</CardTitle>
                            ...
                            {paginatedDoctors.length > 0 ? (
                                paginatedDoctors.map((doctor, index) => (
                                    <TableRow key={doctor.doctorId} className="hover:bg-gray-50">
                                        ...
                                        <TableCell>{doctor.areaOfSpecialization || "N/A"}</TableCell>
